...
// Sửa câu lệnh SQL: Chỉ cần tìm theo username, KHÔNG cần password ở đây
$sql = "SELECT * FROM users1 WHERE username='$username'"; 
$result = mysqli_query($conn, $sql);

// Kiểm tra nếu có dữ liệu mới gán vào biến $user
if ($result && mysqli_num_rows($result) > 0) {
    $user = mysqli_fetch_assoc($result);
} else {
    die("Không tìm thấy thông tin người dùng trong bảng users1.");
}
?>
...
<head>
    <meta charset="UTF-8">
    <title>Trang cá nhân của <?php echo $user['fullname']; ?></title>
    <style>
 * {
